...
export default class IngredientsUsed extends Component {
    state = {
        isLoading: true,
        ingredients: [],
        showEdit: false,
    ...
    getIngredients = () => {
        ...
        const id = this.props.batch.id;
            axios.get(`${API_URL}/ingredients_used?batchID=${id}`, options)
            .then(response => {
                const ingredients = response.data['Ingredients Used'];
                this.setState({
    ...
    submitItem = (event) => {
        ...
        const promise = axios.post(`${API_URL}/ingredients_used`, newIngredient, options)
        this.responseHandler(promise);
    }

    deleteItem = (id) => {
        const { API_URL, options } = this.state;
        const promise = axios.delete(`${API_URL}/ingredients_used/${id}`, options);
        this.responseHandler(promise);
    }
    ...
    renderIngredients = () => {
        ...
        return ingredients.map(ingredient => {
            return <Item key={ingredient.id} item={ingredient} deleteItem={this.deleteItem} />
        })
    }
    ...
    render() {
        const {batch, hideModal} = this.props;
        if (this.state.isLoading) return <></>;
        return (
            <div className='Ingredients'>
                <div className='Ingredients__Header'>
                    <h1 className='Ingredients__Title'>{batch.name} Ingredients Used:</h1>
                ...
                <img className='Ingredients__Icon' onClick={() => hideModal(batch)} src={closeIcon} alt="" />      
